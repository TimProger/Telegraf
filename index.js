...
bot.hears('Comics.hub', (ctx) => {
    ctx.replyWithHTML(`Проект не был закончен до конца, однако над ним я не мало попыхтел. Это мой первый проект, ссылка на который находится ниже. \n\n🤖 https://github.com/TimProger/Comics.hub 🤖`);
})
bot.hears('Telegraf', (ctx) => {
    ctx.replyWithHTML(`Данный проект был закончен и вы можете наблюдать его перед своими глазами. Репозиторий с данным проектом находится по ссылке ниже \n\n🤖 https://github.com/TimProger/Telegraf 🤖`)
})
